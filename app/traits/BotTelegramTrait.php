...
public function request($method, $data, $json_header = 0)
    {
        ...
        curl_setopt_array($ch, [
            ...
            // Фаза соединения (DNS, TCP, TLS) в норме — доли секунды, так что
            // 5 с хватает с запасом, а на мёртвом маршруте бот встанет вдвое
            // меньше. Общий предел оставляем 20 с: через ту же функцию уходят
            // файлы (бэкап, логи, QR), и на медленном канале отправка дольше.
