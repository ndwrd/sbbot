...
public function subscription($return = false)
    {
        ...
        if ($_GET['t'] == 'cl') {
            ...
            // Автообновление подписки у mihomo-клиентов. Заголовок читают
            // графические клиенты (Clash Verge и подобные) при КАЖДОЙ загрузке
            // конфига — и при первом добавлении, и при последующих обновлениях,
            // так что значение можно менять потом, переустанавливать подписку не
            // нужно. Единица — часы (Verge: val * 60 -> минуты). Без него поле
            // "обновление" в клиенте пустое и подписка обновляется только руками.
